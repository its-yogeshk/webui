...
function PaymentPage() {
  ...
  const planId = searchParams.get("userType");
  ...
  const handlePay = async () => {
    ...
    try {
      const { data } = await axios.post(
        `${process.env.NEXT_PUBLIC_BASE_URL_BACKEND_ONE}/orders/create-payment`,
        { amount, userType: planId },
        {
          headers: {
      ...
      const options = {
        ...
        handler: async function (response: any) {
          const verifyRes = await axios.post(
            `${process.env.NEXT_PUBLIC_BASE_URL_BACKEND_ONE}/orders/verify-payment`,
            response,
            {
              headers: {
                Authorization: `Bearer ${token}`,
        ...
        <div className="border rounded-xl p-4 space-y-3 bg-gray-50">
          <div className="flex justify-between">
            ...
            <span className="font-medium text-gray-900">
              {planId === "basic" ? "Silver Plan" : planId === "premium" ? "Gold Plan" : planId}
            </span>
          </div>
